...
async function sessionCreationMiddleware(req, res) {
    try{
        ...
        await addSession(token, {id: user.id, email: user.email, name: user.name, isAnAdmin: user.isAnAdmin, isASeller: user.isASeller, cart: user.cart, wishlist: user.wishlist});
        res.status(200).json({authToken: token});
    }catch(e){
        res.status(500).json({})
    }
}
export default {addSession, getSession, isSessionValid, removeSession, sessionDeletionMiddleware, sessionCreationMiddleware};